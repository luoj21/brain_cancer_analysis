...
manifest_RNA <- read_csv("data/data_files/braincptac3RNAmanifest.csv")
RNA <- read_csv("data/data_files/braincptac3RNA.csv")
protein_shared <- read_csv("data/data_files/braincptac3prodata_shared.csv")
sample_sheet <-  read_csv("data/data_files/braincptac3SAMPLE.csv")
brain_clinical <- read_tsv("data/data_files/brain_clinical.tsv")
...
## Boxplots
bp <- ggplot(gene_subset, aes(Gene, value, fill = type)) + 
...
ggsave("plots/boxplot.png", plot = bp, width = 10, height = 7)
 
 
## Distribution of protein abundances
dist_plot <- ggplot(gene_subset, aes(value, color = type)) + 
   geom_density() + 
...
ggsave("plots/dist_plot.png", plot = dist_plot, width = 10, height = 7)
...
regr <- ggplot(subset(filtered_manifest_RNA, gene_name %in% c("RANBP1", "RANGRF")), aes(fpkm_unstranded, value, color = gene_name)) +
...
ggsave("plots/lin_reg.png", plot = regr, width = 10, height = 7)
